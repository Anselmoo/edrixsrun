...
if __name__ == '__main__':
    """
    Testing if the calculation of the Slater-Condon parameters is working accuratley.
    ...
    F0_d = get_F0('d', F2_d, F4_d)
    G1_dp, G3_dp = 4.0509, 2.3037
    # Averaged dp Coulomb interaction is set to be zero
    F0_dp, F2_dp = get_F0('dp', G1_dp, G3_dp), 7.33495

    # Print Slater-Condon results
    print("F2_d:{}, F4_d:{} -> F0_d:{:2.4}\nG1_dp:{}, G3_dp:{} -> F0_dp:{:2.4}, F2_dp:{:2.4}".format(
        F2_d, F4_d, F0_d,G1_dp, G3_dp, F0_dp, F2_dp))

    #Atomic repulsion of the dd-excitations
    umat_i = get_umat_slater('dp', F0_d, F2_d, F4_d,  # dd
                                    0, 0, 0, 0,  # dp
                                    0, 0)  # pp
    # Atomic repulsion of the pd-excitations
    umat_n = get_umat_slater('dp', F0_d, F2_d, F4_d,  # dd
