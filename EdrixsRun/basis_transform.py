...
if __name__ == '__main__':
    ...
    tmp = np.zeros((5, 5), dtype=np.complex)#Empty 3d-shell as 2D
    case = 't2g'
    if case == 'distortion':
        dt, ds, dq = 0.011428, 0.035714, 0.13
        tmp[0, 0] = 6 * dq - 2 * ds - 6 * dt  # d3z2-r2 equal to dz2
    ...
    if case == 't2g':
        ...
        tmp[3, 3] = 0.3  # dx2-y2
        tmp[4, 4] = 0.3  # dz2
    basis_trans = tmat_r2c('d')  # Get the unitary transformation matrix
    tmp[:, :] = cb_op(tmp, basis_trans)
    print(tmp)
