...
def xas(eval_i, eval_n, T_abs):
    ...
    osc_x = np.abs(T_abs[0, :, 0] * pol[0] + T_abs[1, :, 0] * pol[1] + T_abs[2, :, 0] * pol[2]) **2
    osc_y = np.abs(T_abs[0, :, 1] * pol[0] + T_abs[1, :, 1] * pol[1] + T_abs[2, :, 1] * pol[2]) ** 2
    osc_z = np.abs(T_abs[0, :, 2] * pol[0] + T_abs[1, :, 2] * pol[1] + T_abs[2, :, 2] * pol[2]) ** 2

    plt.stem(eval_n-eval_i[0] + off, osc_x,'k',markerfmt=' ',use_line_collection=True)
    plt.stem(eval_n-eval_i[1] + off, osc_y,'k',markerfmt=' ',use_line_collection=True)
    plt.stem(eval_n-eval_i[2] + off, osc_z,'k',markerfmt=' ',use_line_collection=True)
